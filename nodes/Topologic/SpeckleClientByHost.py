...
class SvSpeckleClientByHost(SverchCustomTreeNode, bpy.types.Node):
	"""
	Triggers: Topologic
	Tooltip: Creates a Speckle Client from the input host parameter   
	"""
	bl_idname = 'SvSpeckleClientByHost'
	bl_label = 'Speckle.ClientByHost'
	Host: StringProperty(name="Host", default="speckle.xyz", update=updateNode)
	Token: StringProperty(name="Token", default="", update=updateNode)
	Replication: EnumProperty(name="Replication", description="Replication", default="Iterate", items=replication, update=updateNode)

	def sv_init(self, context):
		#self.inputs[0].label = 'Auto'
		self.inputs.new('SvStringsSocket', 'Host').prop_name='Host'
	...
	def process(self):
		if not any(socket.is_linked for socket in self.outputs):
			return
		urlList = self.inputs['Host'].sv_get(deepcopy=True)
		...
		self.outputs['Client'].sv_set(outputs)

def register():
    bpy.utils.register_class(SvSpeckleClientByHost)

def unregister():
    bpy.utils.unregister_class(SvSpeckleClientByHost)
