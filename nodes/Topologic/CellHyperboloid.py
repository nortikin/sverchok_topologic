...
def createHyperboloid(baseVertices, topVertices, tol):
	...
	for i in range(0, len(baseVertices)-1):
		...
		faces.append(f)
		w = wireByVertices([baseVertices[i+1], baseVertices[i], topVertices[i+1]])
		f = topologic.Face.ByExternalBoundary(w)
		faces.append(f)
	w = wireByVertices([baseVertices[-1], topVertices[-1], topVertices[0]])
	f = topologic.Face.ByExternalBoundary(w)
	faces.append(f)
...
class SvCellHyperboloid(SverchCustomTreeNode, bpy.types.Node):
	...
	"""
	bl_idname = 'SvCellHyperboloid'
	bl_label = 'Cell.Hyperboloid'
	BaseRadius: FloatProperty(name="Base Radius", default=1, min=0.0001, precision=4, update=updateNode)
	TopRadius: FloatProperty(name="Top Radius", default=1, min=0.001, precision=4, update=updateNode)
	...
	Twist: FloatProperty(name="Twist", default=360, precision=4, update=updateNode)
	...
	def process(self):
		...
		twistList = self.inputs['Twist'].sv_get(deepcopy=True)
		...
		twistList = flatten(twistList)
		tolList = flatten(tolList)
		inputs = [originList, baseRadiusList, topRadiusList, heightList, sidesList, dirXList, dirYList, dirZList, twistList, tolList]
...
def register():
	bpy.utils.register_class(SvCellHyperboloid)

def unregister():
	bpy.utils.unregister_class(SvCellHyperboloid)
