...
class SvEnergyModelReportNames(SverchCustomTreeNode, bpy.types.Node):
    """
    Triggers: Topologic
    Tooltip: Returns the report names from the input energy model
    """
    bl_idname = 'SvEnergyModelReportNames'
    bl_label = 'EnergyModel.ReportNames'
    Replication: EnumProperty(name="Replication", description="Replication", default="Default", items=replication, update=updateNode)

    def sv_init(self, context):
        self.inputs.new('SvStringsSocket', 'Energy Model')
        self.outputs.new('SvStringsSocket', 'Report Names')
    ...
    def process(self):
        ...
        self.outputs['Report Names'].sv_set(outputs)

def register():
    bpy.utils.register_class(SvEnergyModelReportNames)

def unregister():
    bpy.utils.unregister_class(SvEnergyModelReportNames)
