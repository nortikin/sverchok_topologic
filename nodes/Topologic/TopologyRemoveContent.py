...
class SvTopologyRemoveContent(SverchCustomTreeNode, bpy.types.Node):
	...
	"""
	bl_idname = 'SvTopologyRemoveContent'
	bl_label = 'Topology.RemoveContent'
	...
	def process(self):
		...
		contentList = self.inputs['Content'].sv_get(deepcopy=True) #Assume correctly nested.
...
def register():
    bpy.utils.register_class(SvTopologyRemoveContent)

def unregister():
    bpy.utils.unregister_class(SvTopologyRemoveContent)
