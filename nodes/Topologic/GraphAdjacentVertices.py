...
class SvGraphAdjacentVertices(SverchCustomTreeNode, bpy.types.Node):
	"""
	Triggers: Topologic
	Tooltip: Outputs the Vertices adjacent to the input Vertex within the input Graph
	"""
	bl_idname = 'SvGraphAdjacentVertices'
	bl_label = 'Graph.AdjacentVertices'
	...
	def process(self):
		...
		print("Graph Adjacent Vertices Operation consumed "+str(round(end - start,4))+" seconds")
def register():
    bpy.utils.register_class(SvGraphAdjacentVertices)

def unregister():
    bpy.utils.unregister_class(SvGraphAdjacentVertices)
