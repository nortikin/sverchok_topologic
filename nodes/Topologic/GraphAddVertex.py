...
class SvGraphAddVertex(SverchCustomTreeNode, bpy.types.Node):
	"""
	Triggers: Topologic
	Tooltip: Adds the input Vertex to the input Graph
	"""
	bl_idname = 'SvGraphAddVertex'
	bl_label = 'Graph.AddVertex'
	...
	def process(self):
		...
		if ((self.Replication) == "Default"):
			...
			print("Graph Add Vertex Operation consumed "+str(round(end - start,4))+" seconds")
			return
		...
		print("Graph Add Vertex Operation consumed "+str(round(end - start,4))+" seconds")
def register():
    bpy.utils.register_class(SvGraphAddVertex)

def unregister():
    bpy.utils.unregister_class(SvGraphAddVertex)
