...
def processItem(item, overwrite):
	osModel = item[0]
	filepath = item[1]
	# Make sure the file extension is .idf
	ext = filepath[len(filepath)-4:len(filepath)]
	if ext.lower() != ".idf":
		filepath = filepath+".idf"
	...
	w = ft.translateModel(osModel)
	returnStatus = False
	try:
		w.save(openstudio.path(filepath), True)
		returnStatus = True
	except:
		returnStatus = False
	return returnStatus
...
class SvEnergyModelExportToIDF(SverchCustomTreeNode, bpy.types.Node):
	...
	"""
	bl_idname = 'SvEnergyModelExportToIDF'
	bl_label = 'EnergyModel.ExportToIDF'
...
def register():
	bpy.utils.register_class(SvEnergyModelExportToIDF)

def unregister():
	bpy.utils.unregister_class(SvEnergyModelExportToIDF)
