...
class SvSpeckleBranchesByStream(SverchCustomTreeNode, bpy.types.Node):
	"""
	Triggers: Topologic
	Tooltip: Retrieves the Speckle Branches from the input Stream   
	"""
	bl_idname = 'SvSpeckleBranchesByStream'
	bl_label = 'Speckle.BranchesByStream'
	Replication: EnumProperty(name="Replication", description="Replication", default="Iterate", items=replication, update=updateNode)

	def sv_init(self, context):
		self.inputs.new('SvStringsSocket', 'Client')
		self.inputs.new('SvStringsSocket', 'Stream')
		self.outputs.new('SvStringsSocket', 'Branches')
	...
	def process(self):
		...
		self.outputs['Branches'].sv_set(outputs)

def register():
    bpy.utils.register_class(SvSpeckleBranchesByStream)

def unregister():
    bpy.utils.unregister_class(SvSpeckleBranchesByStream)
