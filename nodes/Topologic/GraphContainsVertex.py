...
class SvGraphContainsVertex(SverchCustomTreeNode, bpy.types.Node):
	"""
	Triggers: Topologic
	Tooltip: Outputs True if the input Graph contains the input Vertex. Outputs False otherwise.
	"""
	bl_idname = 'SvGraphContainsVertex'
	bl_label = 'Graph.ContainsVertex'
	...
	def sv_init(self, context):
		...
		self.outputs.new('SvStringsSocket', 'Bool')
	...
	def process(self):
		...
		if ((self.Replication) == "Default"):
			...
			outputs = processItem([graphList[0], vertexList, toleranceList[0]])
		...
		print("Graph Contains Edge Operation consumed "+str(round(end - start,4))+" seconds")
def register():
    bpy.utils.register_class(SvGraphContainsVertex)

def unregister():
    bpy.utils.unregister_class(SvGraphContainsVertex)
