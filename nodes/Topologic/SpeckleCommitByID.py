...
class SvSpeckleCommitByID(SverchCustomTreeNode, bpy.types.Node):
	...
	"""
	bl_idname = 'SvSpeckleCommitByID'
	bl_label = 'Speckle.CommitByID'
	Replication: EnumProperty(name="Replication", description="Replication", default="Iterate", items=replication, update=updateNode)

	def sv_init(self, context):
		self.inputs.new('SvStringsSocket', 'Commits')
	...
	def process(self):
		if not any(socket.is_linked for socket in self.outputs):
			return
		commitList = self.inputs['Commits'].sv_get(deepcopy=True)
		idList = self.inputs['ID'].sv_get(deepcopy=True)
		idList = flatten(idList)
		inputs = [commitList, idList]
...
def register():
    bpy.utils.register_class(SvSpeckleCommitByID)

def unregister():
    bpy.utils.unregister_class(SvSpeckleCommitByID)
