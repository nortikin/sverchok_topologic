...
def processItem(item):
	face = item[0]
	tol = item[1]
	return topologic.FaceUtility.InternalVertex(face, tol)

replication = [("Default", "Default", "", 1),("Trim", "Trim", "", 2),("Iterate", "Iterate", "", 3),("Repeat", "Repeat", "", 4),("Interlace", "Interlace", "", 5)]

class SvFaceInternalVertex(SverchCustomTreeNode, bpy.types.Node):
	"""
	Triggers: Topologic
	...
	def process(self):
		...
		tolList = self.inputs['Tolerance'].sv_get(deepcopy=True)
		faceList = flatten(faceList)
		tolList = flatten(tolList)
		inputs = [faceList, tolList]
		outputs = []
